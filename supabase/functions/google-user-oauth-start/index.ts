...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!clientId) throw new Error("GOOGLE_USER_OAUTH_CLIENT_ID not configured");
    ...
    const params = new URLSearchParams({
      client_id: clientId,
      redirect_uri: `${Deno.env.get("SUPABASE_URL")}/functions/v1/google-user-oauth-callback`,
      response_type: "code",
      scope: SCOPES,
      access_type: "offline",
      prompt: "consent select_account", // always show account chooser, force refresh_token
